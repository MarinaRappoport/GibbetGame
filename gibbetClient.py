...
answer = tkSimpleDialog.askstring('New game?', 'y/n')
if answer == 'n':
    sock.sendall(bytes("GOODBYE"))
    sock.close()
    sys.exit()
...
if data[0]=="GUESS":
    ...
    gotoxy(-250,-150)
    turtle.color("blue")
    turtle.write("Welcome to game Gibbet! I guessed a word of {} letters".format(number_of_letters), font = ("Arial", 14, "normal"))
    
    while True:       
        answer = tkSimpleDialog.askstring("Enter a letter", "You have {} more tries".format(number_of_tries))
        
        sock.sendall(bytes("TRY;{}".format(answer)))
        data = sock.recv(1024).decode()
        data = data.split(';')
        deleteText(-250,-150)
        ...
        if data[0]=="MISTAKE":
            ...
            drawGibbet(9-int(number_of_tries))
