...
class GibbetHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if self.data == "START":
            tryCount = 10
            self.request.sendall(bytes("GUESS;{};{}".format(str(len(secretWord)),tryCount)))
            while True:
                self.data = self.request.recv(1024).decode()
                ...
                if data[0]=="TRY":
                    ...
                    if guess in lettersGuessed:
                        self.request.sendall(bytes("MISTAKE"))
                    elif guess in secretWord:
                        lettersGuessed.append(guess) 
                        guessedWord = getGuessedWord(secretWord, lettersGuessed);
                        
                        if isWordGuessed(secretWord, lettersGuessed):
                            self.request.sendall(bytes("WIN;{}".format(secretWord)))
                            print("Client {} win!".format(self.client_address[0]))
                            break
                        else:    
                            self.request.sendall(bytes("TRUE;{}".format(guessedWord)))
                    
                    else:
                        lettersGuessed.append(guess)    
                        tryCount -= 1
                        if tryCount ==0:
                            self.request.sendall(bytes("FAIL;{}".format(secretWord)))
                            break
                        else:
                            self.request.sendall(bytes("FALSE;{}".format(tryCount)))
